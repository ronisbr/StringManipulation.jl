...
@testset "Prepared text layout" begin
    ...
    @test string_search_per_line(layout, r"a|red") == string_search_per_line(lines, r"a|red")
    ...
    global_active = textview(layout, (1, -1, 1, -1); search_matches = matches, active_match = 1)
    ...
    expected_location = textview(lines, (1, -1, 1, -1); search_matches = matches, active_match = 4)
...
@testset "Prepared ANSI boundaries and transitions" begin
    ...
    @test textview(TextViewLayout([wide_line]), (1, 1, 1, 1)) ==
        (" \e[31m\e[0m", 0, 2)
    @test textview(TextViewLayout([wide_line]), (1, 1, 2, 1)) ==
        ("\e[31m \e[0m", 0, 1)
    ...
    for line in (oracle_lines..., ("AB" * sequence * "C" for sequence in transition_sequences)...)
    ...
    for stride in (1, 2, 32)
        @test textview(
            TextViewLayout([zero_width_ansi]; ansi_checkpoint_stride = stride),
            (1, 1, 2, 1)
